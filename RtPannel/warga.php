...
<body>
                    ...
                    <div class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" role="button" data-bs-toggle="dropdown">
                            <div class="user-avatar me-2">
                                <?php echo strtoupper(substr(is_array($user) && isset($user['username']) ? $user['username'] : 'A', 0, 1)); ?>
                            </div>
                            <span><?php echo htmlspecialchars(is_array($user) && isset($user['username']) ? $user['username'] : 'Admin'); ?></span>
                        </a>
    ...
    <div class="main-content">
        <div class="container">
            ...
            <?php
            $total_warga_row = $conn->query("SELECT COUNT(*) as total FROM warga");
            $total_warga = 0;
            if ($total_warga_row && $total_warga_row->num_rows > 0) {
                $total_warga = $total_warga_row->fetch_assoc()['total'];
            }
            ?>
                        ...
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr>
                                    ...
                                    <td class='align-middle'>" . htmlspecialchars($row['nik']) . "</td>
                                    <td class='align-middle text-center'>
                                        <div class='btn-group' role='group'>
                                            <a href='edit_warga.php?id=" . urlencode($row['id']) . "' class='btn btn-sm btn-warning me-1' title='Edit'>
                                                <i class='fas fa-edit'></i>
                                            </a>
                                            <a href='hapus_warga.php?id=" . urlencode($row['id']) . "' class='btn btn-sm btn-danger' 
